...
for(i in 1:length(datasets_JAV_log)){
  imp <- imputated_data_set_JAV_log[[i]]
  fit <- with(imp, lm(y ~ x1 + x2 + I(x1^2)))
  tab <- summary(pool(fit), "all", conf.int = TRUE)
  lower <- tab$`2.5 %`
  higher <- tab$`97.5 %`
...
for(i in 1:length(datasets_interaktion)){
  imp <- imputated_data_set_JAV_interaktion[[i]]
  fit <- with(imp, lm(y ~ x1 * x2 + x3))
  tab <- summary(pool(fit), "all", conf.int = TRUE)
  lower <- tab$`2.5 %`
  higher <- tab$`97.5 %`
...
diagnostics_ItT_interaction <- test_design %>% 
  group_by(n_obs, missing_mechanism, missing) %>% 
  summarise(coverage_rate = mean(coverage), bias = mean(bias))
...
diagnostics_interaktion_JAV <- test_design %>% 
  group_by(n_obs, missing_mechanism, missing) %>% 
  summarise(coverage_rate = mean(coverage), bias = mean(bias))
