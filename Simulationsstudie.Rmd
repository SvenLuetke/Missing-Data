...
#saveRDS(imputated_data_set_log, file = "imputated_data_log.rds")
...
colnames(test_design)<- c("n_obs","missing_mechanism", "missing", "seed", "coverage", "bias")
...
test_design %>% 
  ...
  summarise(coverage_rate = mean(coverage), bias = mean(bias))
...
for(i in 1:length(datasets_interaktion)){
    ...
    bias <- imputated_data_set_interaktion[[i]]$impDatasets[[1]]$x1 - datasets_interaktion[[i]][[1]]$x1
    test_design[i,6] <- mean(bias)
...
data_completet_dart <- lapply(imputated_data_set_cart, FUN = complete, action = "long", include = FALSE)
data_completet_dart_log <- lapply(imputated_data_set_cart_log, FUN = complete, action = "long", include = FALSE)
data_completet_dart_interaktion <- lapply(imputated_data_set_cart_interaktion, FUN = complete, action = "long", include = FALSE)


test <- as.mids(data_completet_dart)



# calculate bias normal

for(i in 1:length(datasets)){
  bias <- datasets[[i]][[1]] - data_completet_dart[[i]]
}
...
```{r}
##!!!! Name of the datasets is wrong!!!!


# remove x_sqrd from missing dataset

datasets_JAV <- lapply(datasets,function(x){
  dataset <- x[[2]] %>% select(-x_sqrd)
  })

datasets_JAV_log <- lapply(datasets_log,function(x){
  dataset <- x[[2]] %>% select(-x_sqrd)
  })
...
system.time(imputated_data_set_JAV <- parLapply(cl, datasets_JAV, function(x){
  imputations <- mice(x)
}))
...
system.time(imputated_data_set_JAV_log <- parLapply(cl, datasets_JAV_log, function(x){
  imputations <- mice(x)
}))
...
system.time(imputated_data_set_JAV_interaktion <- parLapply(cl, datasets_interaktion, function(x){
...
data_completet_iTt <- lapply(imputated_data_set_JAV, FUN = complete, action = "long", include = TRUE)
data_completet_iTt_log <- lapply(imputated_data_set_JAV_log, FUN = complete, action = "long", include = TRUE)
data_completet_iTt_interaktion <- lapply(imputated_data_set_JAV_interaktion, FUN = complete, action = "long", include = TRUE)

# Erstelle x Squared for all imputated data

for(i in 1:length(datasets_JAV)){
  data_completet_iTt[[i]]$x_sqrd <- data_completet_iTt[[i]]$x1^2
  data_completet_iTt_log[[i]]$x_sqrd <-  data_completet_iTt_log[[i]]$x1^2
}



test <- as.mids(data_completet_iTt[[1]])
